    ...
    sheets.spreadsheets.values.get(
      ...
      (err, response) => {
        ...
        const usersObj = response.values
          .map(row => rowToObject(row, response.values[0]))
          .splice(1)
...
const scrape = (fccUsername, name, country, flag) =>
  new Promise(async (resolve, reject) => {
    try {
      const html = await axios.get(`http://www.freecodecamp.com/${fccUsername}`);
      const $ = cheerio.load(html.data);
      const score = $('h1.flat-top.text-primary')
        .text()
        .substr(2)
        .slice(0, -2);
      const username = $('h1.text-center')
        .first()
        .text();
      const streak = $('h4.col-sm-6.text-left')
        .text()
        .substr(16);
